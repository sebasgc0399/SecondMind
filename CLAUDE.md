...
- **Feature 7 (Capture Multi-Monitor) ✅ (3 rounds de fix):** ventana `/capture` de Tauri Desktop aparece centrada en el monitor del cursor, no siempre en primary. Cubre dos entrypoints: global shortcut `Ctrl+Shift+Space` + menú tray "Captura rápida". **Lógica unificada Rust-side** en `tray.rs::show_capture` (pub(crate)) compartida por ambos handlers. Shortcut registrado Rust-side con `tauri_plugin_global_shortcut::Builder::new().with_handler(...)` dentro de `setup()` — el pattern original (hook JS en `main.tsx`) fue revertido porque `main.tsx` es entrypoint compartido por todas las ventanas → double-register race → callback corría en contexto `capture` operando sobre sí misma en hidden state → quirks Windows tauri#6843. Hit-test cursor-based con `?? monitors[0]` fallback. Dimensiones físicas con `scale_factor * LOGICAL_SIZE` (no `outer_size()`). `set_position` doble (pre + post `show()`) para Windows hidden-window queue quirk. `set_size(LogicalSize(480, 220))` post-show en Rust para resetear tamaño canónico — seguro porque corre al rest. **Drag-from-header REVERTIDO en round 3** por bug tao#3610: cross-DPI drag con `onScaleChanged` + `setSize` causaba feedback loop (setSize → new WM_DPICHANGED → otro setSize → ventana se achica) que paniceaba tao con integer underflow en `event_loop.rs:2035/2042`. Capture es efímera: para mover a otro monitor, re-invocar shortcut. F4 original del SPEC out-of-scope hasta que tao#3610 tenga fix. Capabilities en `capture.json`: `core:window:allow-set-position`, `core:window:allow-set-size`, `core:window:allow-outer-size`. Capabilities `global-shortcut:*` removidas de `default.json` (JS ya no registra). Ver `Spec/features/SPEC-feature-7-multi-monitor-capture.md` con rounds 1/2/3 completos.
