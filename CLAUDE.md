...
- **Feature 7 (Capture Multi-Monitor) ✅:** ventana `/capture` de Tauri Desktop aparece centrada en el monitor del cursor, no siempre en primary. Cubre dos entrypoints: global shortcut `Ctrl+Shift+Space` (JS en `useGlobalShortcutRegistration`) + menú tray "Captura rápida" (Rust en `tray.rs::show_capture`). **Lógica duplicada intencionalmente** en JS y Rust (D1: ~15 LOC por ~1ms de ganancia en latencia del shortcut vs IPC round-trip). Hit-test basado en **cursor**, no "monitor activo" — Windows no tiene concepto claro (D4). Fallback silencioso con `try/catch` JS + `let _ =` Rust: si el reposicionamiento falla, la ventana se muestra igual en su última posición (D3, nunca deja la ventana oculta). `cursorPosition()` + `availableMonitors()` + `PhysicalPosition` son **physical pixels end-to-end** — no multiplicar por `scaleFactor`. Drag manual desde el header (32px `h-8`) via `data-tauri-drag-region` ya existente, habilitado por `core:window:allow-start-dragging`. No persiste posición post-drag (D2: capture efímera, siempre "fresh"). Capabilities nuevas en `capture.json`: `core:window:allow-set-position`, `core:window:allow-outer-size`, `core:window:allow-start-dragging`. `cursorPosition()` + `availableMonitors()` son reads app-level cubiertos por `core:default`. Rust castea `PhysicalPosition<f64>` del cursor contra `PhysicalPosition<i32>` + `PhysicalSize<u32>` de los monitores. Ver `Spec/features/SPEC-feature-7-multi-monitor-capture.md`.
