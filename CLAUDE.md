...
**Producción:** https://app.getsecondmind.co
...
- Cloud Functions desplegadas (17, todas v2): `processInboxItem`, `autoTagNote` (generación BYOK), `onNoteDeleted` (cleanup cascada), `autoPurgeTrash` (scheduled), `generateEmbedding`, `embedQuery` (callable, con allowlist + rate-limit per-uid post-SPEC-51), `saveApiKey`/`deleteApiKey` (callables BYOK), `checkMyAccess` (callable autenticado, gate de allowlist post-auth — reemplazó al público `checkAllowlist` en SPEC-51), `submitAccessRequest`/`listAccessRequests`/`processAccessRequest`/`listAllowlistMembers`/`revokeAccess` (solicitud + gestión de acceso a la beta, SPEC-52/53; `processAccessRequest` además manda el aviso de aprobación vía Resend, SPEC-65 F1), `deleteAccount` (borrado total self-service, SPEC-64), `sendVerificationEmail` (callable autenticada) + `sendResetEmail` (callable pública, anti-enum) — emails de Auth (verify/reset) vía Resend con HTML propio, vía Admin SDK `generateLink` (SPEC-65 F2). Los triggers v1 `onUserCreated`/`onUserDeleted` se removieron en SPEC-53 (junto al counter `userCount`). Detalle de triggers + timeouts en `Spec/ESTADO-ACTUAL.md` § "Cloud Functions"
