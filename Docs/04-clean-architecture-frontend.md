...
**5. Lectura pre-auth sin TinyBase — [`useSignupsEnabled`](../src/hooks/useSignupsEnabled.ts):** corre en `LoginPage` ANTES de autenticar, cuando TinyBase no está inicializado (sus persisters dependen del UID). Lee `config/app.signupsEnabled` con `getDoc` directo del SDK web; `firestore.rules` permite read público de `config/app`. Mismo espíritu que #2 (lectura one-shot) pero en contexto unauth. (SPEC-53 renombró `useSignupCapacity` → `useSignupsEnabled` al pasar a Modelo C: ya no lee `userCount`/`maxUsers`, solo el kill-switch.) A diferencia de useNote/useAuth —exentas por glob en el config— usa `// eslint-disable-next-line no-restricted-imports` inline sobre el único import de `firebase/firestore` del archivo: el disable granular documenta la excepción en el call-site. Gotcha canónico: `Spec/gotchas/tinybase-firestore.md` § "Rutas pre-auth".
