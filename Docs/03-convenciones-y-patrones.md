...
**IDs generados por Firestore, nunca custom.** Excepto `embeddings/{noteId}` que usa el mismo ID de la nota.
...
**Denormalización explícita.** Si un campo se cachea (ej: `sourceTitle` en links), documentar qué Cloud Function lo mantiene en sync.
...
| Vite | Path alias `@/` → `src/` |
| ESLint | `@typescript-eslint/recommended` + import order |
| Prettier | Single quotes, trailing commas, 100 char width |
| Tailwind | Config con tokens de shadcn/ui |
