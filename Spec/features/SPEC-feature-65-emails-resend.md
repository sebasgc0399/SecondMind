...
> **Estado:** F1 **cerrado y en producción**. F2 **implementado + verificado en prod** (smoke 2026-06-22, fix `fc324b8`) — queda **done al mergear el cierre de docs**. **SPEC viva** (no archivar): cabos pendientes — TODO PII de F1, action URL post-beta, F1.4 diferido (**DMARC: publicado `p=none` y verificado por DNS 2026-06-22 → cabo CERRADO**; follow-up menor no bloqueante: `rua` apunta a gmail = destino externo no autorizado por RFC 7489 §7.1 → mover a `dmarc@getsecondmind.co` in-domain; detalle en `gotchas/email-resend.md`). > **Alcance:** SecondMind manda sus emails transaccionales desde un provider propio (Resend), cerrando el loop de postulación (aviso de aprobación) y habilitando emails de Auth con HTML propio.
...
- [ ] Records en Cloudflare (pegando **solo el host**, no el FQDN — Cloudflare auto-appendea la zona):
  ...
  - `TXT` `_dmarc` → `v=DMARC1; p=none; rua=mailto:sebasgc0399@gmail.com; fo=1` (recomendado, no obligatorio para verificar). **✅ Publicado y verificado por DNS el 2026-06-22** (un solo `_dmarc`, sin duplicado, en DNS-only; resolvers 1.1.1.1 + 8.8.8.8). Caveat del `rua` externo en `gotchas/email-resend.md`.
