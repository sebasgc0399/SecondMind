...
| F2.7 | constantes + persistidos D5     | `discardableEntries`, `useOnboarding` steps, labels de enums (PRIORITY/TYPE/STATUS_LABELS dispersos) → `src/lib/entityLabels.ts` central (incluye AREAS/HABITS en render, D5); **`ENTITY_LABELS` de `usePendingSyncCount` + migración de `usePendingSyncCount.test.ts`** (diferidos desde F2.1: el popover del PendingSyncIndicator muestra entidades es hasta acá); defaults "Sin título" al crear (`inboxRepo`) → `t()` al momento de creación (D5); templates literature/permanent → función que materializa el JSONContent con `t()` al insertar (**manual, nunca instrument**); D6 |
