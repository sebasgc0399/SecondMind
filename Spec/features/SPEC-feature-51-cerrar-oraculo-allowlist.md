...
- **🐞 Bug de follow-up (b) — `/verify-email` muestra "Enviamos un enlace" sin enviarlo:** detectado en E2E del caso signup email/pw + red caída en el gate (F3). Cuando `enforceAccessGate` lanza `access-check-unavailable` **antes** de `sendEmailVerification`, la cuenta queda creada+autenticada → `Layout` redirige a `/verify-email`, que afirma incondicionalmente que el correo se envió. Mensaje **falso** en ese path. Acotado (no rompe seguridad; "Reenviar enlace" recupera). Fix candidato: que `/verify-email` no afirme el envío sin confirmar, o que el caso-red de signup no navegue a verify-email. **Dimensionar con el bloque `signUpWithEmail` después de cerrar este SPEC.**
