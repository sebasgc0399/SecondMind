...
> **Estado:** 🔨 **En implementación** (branch `feat/harness-e2e-callables`). Plan aprobado (step 2 SDD) en `~/.claude/plans/quizzical-tinkering-dijkstra.md`. Generado desde `Spec/drafts/DRAFT-harness-e2e-callables.md` (draft eliminado). **Correcciones del plan incorporadas:** (1) F7 (canario) antes de F4; (2) `test:rules` en CI = D8 (PR aparte, previo); (3) `emailVerified` fuera de F5 (ninguna callable lo lee); (4) inyección de `ADMIN_EMAIL` = `.secret.local` **generado en runtime** (D5, ver F4); (5) type-check de `e2e/` vía `tsconfig.e2e.json` + step de CI dedicado (el `tsc --noEmit` de CI es no-op → D9, PR aparte).
