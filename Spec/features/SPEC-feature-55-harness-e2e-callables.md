...
> **Pendientes:** batch 2 (BYOK `saveApiKey`/`deleteApiKey`, requiere migrar a imports modulares). **D8** (`test:rules` en CI) y **D9** (`tsc --noEmit` → `tsc -b`) **cerrados** — PRs aparte, CI verde.
