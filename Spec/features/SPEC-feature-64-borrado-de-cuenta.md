...
| **D7** | **Tormenta de triggers `onNoteDeleted`: dejar correr (idempotente) en v1** | El `recursiveDelete` de `users/{uid}` dispara `onNoteDeleted` por nota; su trabajo (borrar embedding+links) es **redundante** con el wipe pero **idempotente** (borra docs ya borrados = no-op). A escala beta (≤100 users, conteos modestos) es aceptable; `maxInstances` de `onNoteDeleted` acota la concurrencia. **Supresión por marker** (short-circuit del trigger durante el wipe) = optimización documentada para después, NO v1. **Validar el costo/comportamiento real en step 2 (Plan mode).**                                                                                                                                                   |
...
- **Qué:** nuevo callable autenticado. Pasos, **idempotente y re-ejecutable** (puede fallar a mitad y re-correrse):
  1. `requireAuth` → `uid`, `email` (del token, normalizado `trim().toLowerCase()` para las keys `allowlist`/`accessRequests`).
  ...
  5. `db.doc('allowlist/'+email).delete()` (D4c/D5, idempotente).
...
- **Smoke (política de datos QA, CLAUDE.md step 5):** contra una **cuenta throwaway** (NUNCA la cuenta real de Sebastián — este flujo es irreversible y borra Auth): crear, poblar, borrar, verificar server-side (`getDocFromServer` / Firebase MCP) que no quedó nada + Auth user ausente. Smoke nativo: en el APK (Android), el botón abre `app.getsecondmind.co/settings` en el navegador (Tauri queda fuera de v1).

## Riesgos / cuestiones abiertas

- **D7 — tormenta de `onNoteDeleted` en wipe masivo:** validar costo/latencia reales en step 2; si molesta a escala, supresión por marker (fuera de v1). No es bloqueante (idempotente).
