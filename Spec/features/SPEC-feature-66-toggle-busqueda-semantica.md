...
> **Estado:** ✅ **EJECUTADO y LIVE en v0.6.0** (2026-06-29). Este archivo es el **plan original** (formato prescriptivo, SDD step 2); la feature se implementó en su **versión endurecida** bajo **SPEC-68 (consent server-authoritative)** — el gate server-side de egreso a OpenAI, el callable `markSemanticConsent`, el modal de reconocimiento (`SemanticConsentModal`), el toggle en settings (`SemanticSearchSection`) y el backfill están deployados (web/desktop/Android). **El registro de implementación es [SPEC-68](SPEC-feature-68-consent-server-authoritative.md)** + los commits `feat(consent):…` del release; este plan se conserva por trazabilidad histórica y **no se reescribe** (evitar duplicar SPEC-68). **Decisiones D1–D6 CERRADAS** (abajo).
