...
- [ ] **Gate Web (F5):** criterios verdes — lectura offline, **first-offline-boot sin cache = vacío usable (no colgado)**, **replay Caso A (post-flush)** verificado en server, **multi-tab (2 pestañas, sin `failed-precondition`)**, corrupción degrada limpio, init-failure degrada limpio, revert probado. **Caso B (reload inmediato)** medido (no bloqueante, D11).
