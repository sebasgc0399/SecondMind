...
> Última actualización: Feature 42 Polish offline UX y visibilidad per-item (Mayo 2026). F42.1 hook `usePendingSyncForEntity(type, id)` + `<PendingSyncDot>` con cache `(entityType, id)` preservando ref + `matchesId` para composite keys; dot ámbar (pending) / destructive (error) en 5 cards (note/inbox/task/project/objective; TaskCard con `right-12 top-1` por margin negativo del botón de check). F42.2 `<DiscardPendingDialog>` confirmación grouped por entidad antes de `clear()` queues + `delRow` ANTES en createsQueueBindings (G4 F30); helper `getDiscardableEntries()` snapshot one-shot con dedupe `(entityType, id)` y label resolvers per-entity. F42.3 `<PendingSyncIndicator>` unificado con severity discriminada `error > offline > pending` + chipColors/dotColor switch, retry disabled offline o sin errors; deprecada `<OfflineBadge>`. F42.4 InboxItem placeholder "En cola" cuando offline + AI pendiente, fallback de spinner "Procesando con AI...". F42.5 Avatar fallback automático con `imgError` state + `useEffect` reset on `[user.photoURL]` (cubre CORS/redirect Google en Capacitor WebView, signed URLs expirados, network mid-load). 171/171 tests pasando (+10 vs F38 con `usePendingSyncForEntity.test.ts` nuevo + 2 tests del PendingSyncIndicator). E2E Playwright validado en 4 viewports (Desktop 1280, Tablet 768, Mobile 375, Samsung S23 Ultra 412×915). 2 polishes UX post-QA: suavizado del color destructive del dialog (`bg-destructive/10` + dark mode override) y eliminación del Dialog.Close redundante (Cancelar + Escape + click backdrop ya cubren la afordancia). 2 gotchas escalados (avatar onError → `gotchas/ui-componentes.md`, hook per-entity → `gotchas/tinybase-firestore.md`). QuickCaptureProvider sigue con `inboxStore.setRow` directo (D4 F38.3) — items capturados no se marcan como pending (limitación documentada D2 F42.1).
