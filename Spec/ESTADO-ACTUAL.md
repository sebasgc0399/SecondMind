...
> Última actualización: Feature 7 Capture Multi-Monitor (Abril 2026)
...
- **Feature 6 — Theme System + Paleta Violet Desaturada**: 3 modos de color (Claro/Automático/Oscuro) con selector visual en Settings, paleta oklch violet hue 285° (chroma 0.005–0.12 desaturado tipo Linear/Raycast), script inline anti-flash en `<head>` de `index.html`, hook `useTheme` con `useSyncExternalStore` (sin Provider, DOM class como fuente de verdad). Default = `auto` (desvío del SPEC D3 que decía `dark` — el audit reveló que la app NO tenía `.dark` setter en ningún lado y rendería `:root` light por default, así que `auto` es la opción de mínima sorpresa). `@custom-variant` migrado a canonical shadcn `(&:where(.dark, .dark *))` (el `(&:is(.dark *))` previo no matcheaba `<html class="dark">` por sí mismo). `color-scheme: light/dark` property agregada (scrollbars nativos + autocomplete + form controls respetan tema). Tokens nuevos: `--background-deep` (backdrop modals), `--border-strong` (bordes modals), `--shadow-modal` (reemplaza `rgba(0,0,0,0.5)` hardcoded que generaba halo elefante en light), `--graph-*` categóricos. Reagraph usa Three.js que NO procesa oklch — graph colors migrados a hex hardcoded en `src/lib/theme-colors.ts` con comentario KEEP IN SYNC vs `index.css`. `<GraphCanvas>` recibe `theme` prop dinámico (canvas.background, node.label.color, edge.fill, arrow.fill) — sin esto el canvas era blanco hardcoded en dark mode. Scope expandido incluye fixes preexistentes: `dark:` variants faltantes en NoteCard/SummaryL3/ReviewBanner, `shadow-modal` token en 5 modals. ThemeSelector CSS-only con mini-previews (clip-path diagonal para "Auto"), min-h 112 para tap target 44, aria-pressed.
- **Feature 7 — Capture Multi-Monitor**: la ventana `/capture` (Tauri Desktop) aparece centrada en el monitor que contiene el cursor, no siempre en el primario. Cubre los dos entrypoints: global shortcut `Ctrl+Shift+Space` (JS en `useGlobalShortcutRegistration`) y menú del system tray → "Captura rápida" (Rust en `tray.rs::show_capture`). Lógica duplicada intencionalmente en JS y Rust (D1) para evitar IPC round-trip desde el handler del tray — 15 LOC por ~1ms de ganancia en latencia de shortcut. Hit-test basado en cursor, no "monitor activo" (Windows no tiene concepto claro — D4). Fallback silencioso con `try/catch` en JS y `let _ =` en Rust: si el reposicionamiento falla, la ventana se muestra igual en su última posición (peor caso = comportamiento pre-fix, nunca oculta — D3). Drag manual desde el header de capture (32px) vía `data-tauri-drag-region` ya existente, habilitado por la capability `core:window:allow-start-dragging`. No persiste posición post-drag (contradice D2: capture es efímera, siempre "fresh"). Capabilities nuevas en `capture.json`: `core:window:allow-set-position`, `core:window:allow-outer-size`, `core:window:allow-start-dragging`. `cursorPosition()` y `availableMonitors()` son reads app-level que Tauri v2 considera parte de `core:default` (no requieren permiso explícito). Ver `Spec/features/SPEC-feature-7-multi-monitor-capture.md`.
...
- **Global shortcut registrado Rust-side via `with_handler` dentro de `setup()` (cambio post-F7).** Pattern anterior (hook JS en `main.tsx`) se montaba en AMBAS ventanas porque `main.tsx` es entrypoint compartido → double-register race donde el callback final quedaba registrado en el contexto de la ventana `capture` operando sobre sí misma en hidden state. Windows tiene quirks conocidos en este escenario (tauri-apps/tauri#6843). Rust-side con `with_handler(move |app, shortcut, event| { ... })` garantiza registro único, contexto AppHandle estable, y permite compartir la función `show_capture` con el handler del tray (zero duplicación).
...
- **`data-tauri-drag-region`** NO se usa en la capture window (revertido en round 3 de F7). La feature F4 original del SPEC planteaba drag desde el header, pero arrastrar entre monitores con DPI distinto disparaba una feedback loop de `onScaleChanged` → `setSize` → nuevo `WM_DPICHANGED` que eventualmente paniceaba `tao` con integer overflow (`attempt to subtract with overflow` en `event_loop.rs:2035`). Capture es efímera por diseño — para mover a otro monitor, re-invocar `Ctrl+Shift+Space` con el cursor ahí (Rust-side position detection lo resuelve). Drag-from-header queda fuera de scope hasta que tao resuelva el bug (issue tauri-apps/tauri#3610 lleva 4 años abierto).
- **Capture window centra en monitor del cursor, no en primary (Feature 7).** Sin este fix, `tauri.conf.json` con `"center": true` + `window_state` con denylist `["capture"]` producían "siempre en primario" en setups multi-monitor. La lógica está duplicada en JS (`useGlobalShortcutRegistration` para `Ctrl+Shift+Space`) y Rust (`tray.rs::show_capture` para el menú del tray) — D1 del SPEC prioriza latencia del shortcut sobre DRY (evita IPC round-trip). Hit-test del monitor usa `cursor.x >= pos.x && cursor.x < pos.x + size.width` (lo mismo para Y) con `?? monitors[0]` como último recurso.
- **Dimensiones físicas de la capture window se calculan con `scaleFactor` del monitor destino, NO con `outerSize()` (fix post-F7).** Sintoma sin este fix: tras un drag cross-DPI, `outerSize()` devolvía un tamaño stale que metía el cálculo de centrado en un loop de escalado incorrecto. Correcto: `winWidth = CAPTURE_LOGICAL_WIDTH * target.scaleFactor`, `winHeight = CAPTURE_LOGICAL_HEIGHT * target.scaleFactor`. Constantes canónicas 480×220 definidas en JS (`useGlobalShortcutRegistration.ts`) y Rust (`tray.rs`).
- **`setPosition()` en hidden windows es inconsistente en Windows (fix post-F7).** La queue no se aplicaba antes de `show()`, causando que el shortcut abriera siempre en la última posición visible ignorando el cursor. Workaround: llamar `setPosition()` DOS veces — una pre-`show()` y otra post-`show()`. El segundo call es el definitivo; el primero intenta el reposicionamiento "hidden" (funciona en macOS/Linux, queue en Windows).
- **`setSize(LogicalSize)` post-`show()` fuerza reflow de WebView2 tras cross-DPI drag (fix post-F7).** Bug observado: al arrastrar capture entre monitores con DPI distinto, Tauri/tao auto-resizea el chrome para mantener LogicalSize, pero WebView2 no rescala su contenido interno → aparecen scrollbars horizontales/verticales y clipping. Reset explícito con `setSize(LogicalSize(480, 220))` después del `show()` fuerza a WebView2 a reflow en el DPI del monitor actual. Aplicar en cualquier ventana Tauri frameless que pueda cruzar monitores.
- **Cambio de `try/catch` silencioso a `console.error` en el callback JS (fix post-F7).** La versión original del SPEC tenía `catch { /* silent */ }` que ocultó los dos bugs DPI por varias pruebas — diagnosticar requirió adivinar. Regla: fallbacks silenciosos son aceptables para comportamiento, NO para diagnóstico — siempre loggear con contexto. Rust sigue con `let _ =` porque los errores ahí van a los logs del plugin automáticamente.
- **`onScaleChanged` listener + `setSize(LogicalSize)` es una trampa en Windows (gotcha aprendido en round 3 de F7).** El pattern parece correcto pero crea una feedback loop: setSize aplicado mid-drag cuando la ventana straddlea monitores dispara otro `WM_DPICHANGED` → otro setSize → la ventana se achica iterativamente hasta desaparecer → `tao` panicea con integer underflow en `event_loop.rs:2035/2042`. No hay forma de debouncear seguro sin añadir complejidad significativa. Conclusión: **no intentar reaccionar a `WM_DPICHANGED` con `setSize` en ventanas draggeables**. Solución real: no permitir drag si la ventana es efímera, o aceptar el glitch visual si el drag cross-DPI es requisito.
- **Hook JS en `main.tsx` se monta en TODAS las ventanas del bundle.** Tauri no separa entrypoints por ventana — `main` y `capture` comparten `main.tsx`. Cualquier side effect en `main.tsx` (registro de shortcut global, listeners de eventos de app, init de servicios singleton) corre en ambos contextos. Hooks que deben correr una sola vez deben registrarse Rust-side O tener guard por `getCurrentWebviewWindow().label`. `useCloseToTray` está bien porque cada ventana necesita su propio listener de close; `useGlobalShortcutRegistration` NO estaba bien por eso se movió a Rust.
...
11.24 **`@custom-variant dark (&:is(.dark *))` NO matchea `<html class="dark">` por sí mismo**, solo descendientes. Usar el pattern canónico shadcn `(&:where(.dark, .dark *))`. Hoy funcionaba "por suerte" porque el body hereda tokens vía `@apply bg-background`, pero era un bug latente — aplicar `dark:*` directamente al `<html>` o a un `<body>` sin `html.dark body` como selector rompía. Aplicado en F6 como commit 1 aislado.

11.25 **`color-scheme: light/dark` property obligatoria** en `:root`/`.dark` para que scrollbars nativos, autocomplete y form controls respeten el tema. Sin él, Tauri WebView2 y Capacitor Android muestran scrollbars del OS por default ignorando el tema de la app.

11.26 **Three.js NO procesa strings `oklch()`.** Solo acepta hex, rgb, nombres CSS. Reagraph pasa el `fill` de `GraphNode` directo a Three como `ColorRepresentation`. Workaround en F6: `src/lib/theme-colors.ts` mantiene hex strings equivalentes de los `--graph-*` oklch con comentario `// KEEP IN SYNC WITH src/index.css`. Cualquier feature futura que pase colores a WebGL (Three, regl, pixi) debe convertir oklch a hex antes.

11.27 **Reagraph `<GraphCanvas>` tiene canvas blanco hardcoded** si no se pasa `theme` prop con `canvas.background`. Bug visible en dark mode (rectángulo blanco sobre fondo oscuro). Solución: construir `ReagraphTheme` dinámico con `useMemo` dep en `resolvedTheme`, extendiendo `lightTheme` base y overridando `canvas.background`, `node.label.color`, `edge.fill`, `arrow.fill`. Import: `import { lightTheme } from 'reagraph'` (también existe `darkTheme` pero usamos el light como base y overridamos los 4 campos según tema para no mezclar opacidades/defaults).

11.28 **Script inline anti-flash en `<head>` antes del `<script type="module">`.** Es la única forma confiable de evitar flash de tema incorrecto en Vite SPA. Los modules se defer-loadean — el browser renderiza antes de que corran. El script inline clásico (no `type="module"`) ejecuta sincrónicamente durante el parsing del HTML. Try-catch defensivo para Capacitor/Tauri/iframe edge cases donde `localStorage.getItem` puede tirar. IIFE para no polucionar global scope.

11.29 **`resolvedTheme` snapshot DEBE leer del DOM (`classList.contains('dark')`) no recalcular** de `localStorage + matchMedia`. Si recomputás, hay drift con el script inline que ya aplicó la clase antes de React mount. El snapshot del DOM hace que `resolvedTheme` sea auto-correcto incluso si algo externo modifica la clase.

11.30 **Custom event `sm-theme-change` para notificación same-tab.** `storage` event solo dispara cross-tab (cuando OTRO tab escribe al localStorage). `setTheme` en el mismo tab no dispara `storage` — por eso el hook escucha además el custom event que el setter dispatch. Patrón generalizable para cualquier hook que persista en localStorage y necesite reactividad same-tab.

11.31 **localStorage del tema es per-plataforma, no per-cuenta.** Preferencia de dispositivo (user puede querer dark en laptop, light en teléfono). Capacitor serve desde `capacitor://localhost` — origen distinto al web deploy, localStorage separado. Tauri también separado. Este es comportamiento correcto; si en el futuro se quiere sync cross-device, agregar capa Firestore encima sin romper lo existente.

11.32 **Default theme = `auto`, no `dark`.** La app NO tenía setter de `.dark` pre-F6 — rendía `:root` (light) para todos los usuarios. Cambiar default a `dark` al deploy sería un cambio impuesto. `auto` respeta `prefers-color-scheme` del sistema y minimiza sorpresa. Desvío justificado del SPEC D3.

11.33 **Token `--shadow-modal` varía por tema.** En dark, sombra negra sobre fondo oscuro se desvanece. En light, `rgba(0,0,0,0.5)` sobre blanco = halo gris elefante anti-premium. Light usa `oklch(0.3 0.04 285 / 0.22)` (violet-tinted soft), dark usa `oklch(0 0 0 / 0.55)`. Cualquier Dialog/Popup nuevo debe usar `shadow-modal` en vez de hardcoded `shadow-[...]`.

11.34 **Tokens `--background-deep` y `--border-strong` son usados por 5 modals** (ObjectiveCreateModal, QuickCapture, ProjectCreateModal, CommandPalette, NoteLinkModal). Antes de F6 NO existían en `index.css` y Tailwind resolvía a `undefined` → backdrops bogus. F6 los define y mapea en `@theme inline`. Cualquier Dialog nuevo con backdrop debería usar `bg-background-deep/80` + `border-border-strong`.
...
30. **Capture window siempre abre en primary si no se reposiciona manualmente (Feature 7).** `tauri.conf.json` con `"center": true` significa "centrar en primary", no "centrar en el monitor activo". `window_state` con denylist `["capture"]` impide persistir posición (diseño intencional: capture efímera siempre fresh). El fix requiere `setPosition()` con `PhysicalPosition` calculado desde `cursorPosition()` + `availableMonitors()`, duplicado en JS (hook del shortcut) y Rust (handler del tray). No hay forma de delegar — sin duplicación, o el tray pierde el fix o el shortcut se vuelve más lento por IPC.

31. **Hidden-window `setPosition()` en Windows se queue sin aplicar (fix post-F7).** Sintoma: la primera versión de F7 mostraba el shortcut siempre en la última posición visible. Root cause: Windows solo respeta cambios de posición en ventanas visibles; las llamadas a setPosition en `visible: false` entran en cola pero `show()` no las consume antes de renderizar. Workaround confirmado: llamar `setPosition()` dos veces — una pre-show (inocua en macOS/Linux, queue en Windows) y una post-show (definitiva). Aplicar en cualquier ventana con patrón "hide → show → reposicionar".

32. **WebView2 no rescala contenido tras cross-DPI drag (known issue tauri-apps/tauri#3610).** Tauri/tao handlea WM_DPICHANGED para el chrome pero WebView2 no rescala su contenido interno → scrollbars + clipping. Issue upstream abierto desde 2022 sin fix. **Round 3 de F7 decidió eliminar el drag de capture window** porque intentar corregir via `onScaleChanged` + `setSize` dispara una feedback loop que panicea tao con integer underflow. Para ventanas efímeras donde drag no es esencial → remover `data-tauri-drag-region`. Para ventanas donde drag es requisito → documentar la limitación o limitar a monitores con mismo DPI.

33. **Hook JS en `main.tsx` se monta en TODAS las ventanas del bundle (lección aprendida post-F7).** Tauri no separa entrypoints por ventana — `main`, `capture` y cualquier ventana futura comparten el mismo `main.tsx`. Side effects globales (registro de shortcut OS-level, init de servicio singleton, analytics) duplican registro si no tienen guard. Patrones seguros:
    1. Registrar Rust-side si la feature es OS-level (shortcuts globales, tray listeners).
    2. Guard `getCurrentWebviewWindow().label === 'main'` en el hook si hay que quedarse en JS.
    3. Hook legítimamente per-window: `useCloseToTray` está bien porque cada ventana necesita su propio listener de `close-requested`.
...
**Feature 6 (Theme System) ✅ completada.** Sistema de tema con 3 modos (Claro/Automático/Oscuro), paleta oklch violet desaturado hue 285°, selector visual en Settings, script inline anti-flash, `useTheme` hook sin Provider. Default `auto`. Fixes preexistentes incluidos: tokens `--background-deep`/`--border-strong`/`--shadow-modal` + graph colors theme-aware (Reagraph + Three.js con hex strings) + 4 `dark:` variants faltantes.

Próximas iteraciones candidatas:

- **Temas de acento** — permitir elegir color de acento (azul/verde/naranja) cambiando solo `--primary-*` tokens. Fondos/borders/muted no cambian.
- **Sync de preferencias cross-device** via Firestore `users/{uid}/preferences.theme` — si demanda real aparece. Hoy es per-dispositivo (correcto).
- **Actualizar `theme_color` del PWA manifest dinámicamente según tema** — hoy hardcoded a `#878bf9`, out of F6 scope.
- **Visual regression baselines** con Playwright si se introduce screenshot testing — F6 es el primer snapshot post-paleta.
