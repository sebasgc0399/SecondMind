...
- **Feature 7 — Capture Multi-Monitor**: la ventana `/capture` (Tauri Desktop) aparece centrada en el monitor que contiene el cursor, no siempre en el primario. Cubre los dos entrypoints: global shortcut `Ctrl+Shift+Space` y menú del system tray → "Captura rápida". **Lógica unificada Rust-side** en `tray.rs::show_capture` (pub(crate)) llamada desde `lib.rs` por el handler del shortcut global (`tauri_plugin_global_shortcut::Builder::new().with_handler(...)` dentro de `setup()`) y desde el handler del tray. Un solo source of truth. El pattern original del SPEC (hook JS en `main.tsx`) fue revertido porque `main.tsx` es entrypoint compartido por `main` y `capture` → el hook se montaba dos veces → double-register race → el callback terminaba corriendo en el contexto de `capture` operando sobre sí misma en hidden state → quirks conocidos de Windows (tauri-apps/tauri#6843) que ni double setPosition resolvía. Hit-test basado en cursor: `cursor.x >= pos.x && cursor.x < pos.x + size.width` con `?? monitors[0]` fallback. Dimensiones físicas calculadas con `scaleFactor` del monitor destino (NO `outerSize()` que queda stale). setPosition se llama DOS veces (pre y post show) porque en Windows hidden-window setPosition queue-sin-aplicar. **Drag desde el header fue revertido en round 3** por bug de tao en cross-DPI drag (feedback loop → panic). Capture es efímera: para mover a otro monitor, re-invocar el shortcut con el cursor ahí. Capabilities en `capture.json`: `core:window:allow-set-position`, `core:window:allow-set-size`, `core:window:allow-outer-size`. Ver `Spec/features/SPEC-feature-7-multi-monitor-capture.md` con rounds 1/2/3 documentados.
...
- **Capture window centra en monitor del cursor (Feature 7).** Sin este fix, `tauri.conf.json` con `"center": true` + `window_state` con denylist `["capture"]` producían "siempre en primary". La lógica vive en `tray.rs::show_capture` (pub(crate)) — un único point-of-truth compartido por shortcut handler (registrado Rust-side con `with_handler` dentro de `setup()`) y tray menu. Hit-test: `cursor.x >= pos.x && cursor.x < pos.x + size.width` (lo mismo para Y) con `?? monitors[0]` como último recurso. Dimensiones físicas calculadas con `scale_factor * LOGICAL_SIZE`, NO `outer_size()` (quedaba stale post cross-DPI drift, ahora moot porque drag fue revertido pero el patrón se mantiene por robustez).
- **`set_position()` en hidden windows es inconsistente en Windows (fix post-F7).** Queue sin aplicar antes de `show()`, causando que el shortcut abriera siempre en la última posición visible. Workaround: llamar `set_position()` DOS veces — pre-`show()` (inocua en macOS/Linux, queue en Windows) y post-`show()` (definitiva). Aplicar en cualquier ventana con patrón "hide → show → reposicionar".
- **`set_size(LogicalSize(480, 220))` post-`show()` en Rust para resetear tamaño canónico.** Seguro porque corre al rest (ventana ya visible en monitor destino, no mid-drag). El pattern análogo en JS (`onScaleChanged` listener llamando `setSize`) era una trampa que causaba feedback loop → tao panic — por eso drag de capture fue revertido.
...
30. **Capture window siempre abre en primary si no se reposiciona manualmente (Feature 7).** `tauri.conf.json` con `"center": true` significa "centrar en primary", no "centrar en el monitor activo". `window_state` con denylist `["capture"]` impide persistir posición (diseño intencional). Fix: `set_position()` con `PhysicalPosition` calculado desde `cursor_position()` + `available_monitors()`, Rust-side en `tray.rs::show_capture`, llamado tanto por el global shortcut (registrado en `lib.rs::setup` con `with_handler`) como por el tray menu.

31. **Hidden-window `set_position()` en Windows se queue sin aplicar (fix post-F7).** La primera versión de F7 registraba el shortcut en JS y llamaba setPosition una vez pre-show — Windows lo dejaba en cola y la ventana abría en la última posición visible. Workaround actual: `set_position()` DOS veces (pre y post `show()`) desde Rust. Aplicar en cualquier ventana con patrón "hide → show → reposicionar".

32. **WebView2 no rescala contenido tras cross-DPI drag (known issue tauri-apps/tauri#3610).** Tauri/tao handlea WM_DPICHANGED para el chrome pero WebView2 no rescala su contenido interno → scrollbars + clipping. Issue upstream abierto desde 2022 sin fix. **Round 3 de F7 decidió eliminar el drag de capture window** porque intentar corregir via `onScaleChanged` + `setSize` dispara una feedback loop (setSize → nuevo WM_DPICHANGED → otro setSize → repeat) que eventualmente panicea `tao` con integer underflow (`attempt to subtract with overflow` en `event_loop.rs:2035/2042`). Para ventanas efímeras donde drag no es esencial → remover `data-tauri-drag-region`. Para ventanas donde drag es requisito → limitar a monitores con mismo DPI.
...
33. **Hook JS en `main.tsx` se monta en TODAS las ventanas del bundle (lección aprendida post-F7).** Tauri no separa entrypoints por ventana — `main`, `capture` y cualquier ventana futura comparten el mismo `main.tsx`. Side effects globales (registro de shortcut OS-level, init de servicio singleton, analytics) duplican registro si no tienen guard. Patrones seguros:
    1. Registrar Rust-side si la feature es OS-level (shortcuts globales, tray listeners). F7 terminó acá tras el round 2.
