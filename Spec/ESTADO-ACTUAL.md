...
> Última actualización: Feature 43 Limpieza de navegación mobile (Mayo 2026). F43.1 CTA "Capturar" (botón violeta full-width) eliminado de `<SidebarContent />` — shared component entre `<Sidebar />` desktop y `<NavigationDrawer />` mobile, así que el cleanup afecta cross-platform por construcción (desktop expanded + tablet collapsed + mobile/tablet drawer); junto se removieron handler `handleCaptureClick`, hook `useQuickCapture` y `Plus` icon. `navItems.ts` reorganizado: Inbox movido a sección "Ejecución" posición 2 (post-Dashboard) eliminando la sección "Captura" entera (regla operativa "ninguna sección de drawer con un único hijo"). F43.2 `<BottomNav />` reducido de 5 a 4 slots fijos (Dashboard / Notas / Tareas / Inbox con badge), eliminando slot "Más" + prop `onMoreClick` + import `MoreHorizontal`; `<MoreDrawer />` borrado completamente con su mount/state/import en `app/layout.tsx`. FAB persiste como única primary action mobile (Material 3) y su position con `bottom: calc(80px + 16px + var(--sai-bottom))` es invariante al slot count (depende de altura BottomNav fija `calc(64px + var(--sai-bottom))`, no del número de items). Antipatrones eliminados: doble overflow (BottomNav slot "Más" + NavigationDrawer cubrían el mismo rol) y duplicación primary action (FAB + CTA Capturar). 171/171 tests pasando, build limpio, lint sin errores nuevos, E2E Playwright validado en 4 viewports: mobile 390×844 (4 slots × 98×63px WCAG-compliant, drawer limpio con Inbox bajo Ejecución pos 2, sin sección Captura, sin CTA), tablet 768 (sidebar collapsed sin icono CTA), desktop 1280 (sidebar expanded sin CTA, Inbox pos 2 reordenado). D6 trade-off documentado: tablet (768-1023px) y desktop con sidebar visible quedan con sólo shortcut Alt+N para QuickCapture (TopBar QuickCaptureButton se renderiza sólo con `sidebarHidden=true`, FAB es mobile-only); aceptado por uso minoritario, escalable a F44 si surge feedback de discoverability. Sin gotchas técnicos escalados — F43 fue cleanup mecánico sin sorpresas; lecciones (SidebarContent shared cross-platform; FAB invariante al slot count; regla "ninguna sección con un único hijo"; tablet sin FAB ni TopBar) archivadas en el SPEC.
