...
> Última actualización: Feature 44 Fix CSP `cloudfunctions.net` Tauri (Mayo 2026). F44.1 agregó `https://us-central1-secondmindv1.cloudfunctions.net` al `connect-src` de [`src-tauri/tauri.conf.json`](../src-tauri/tauri.conf.json) (línea 43) — desbloquea `httpsCallable(embedQuery)` desde la app desktop; la CSP enumeraba `identitytoolkit`/`securetoken`/`*.googleapis.com`/`*.firebaseio.com` pero omitía `cloudfunctions.net` donde viven las CFs callables. Bug Tauri-only (PWA web y Capacitor Android no aplican CSP custom), latente desde F3 búsqueda híbrida porque `embedQuery` era la primera y única callable consumida desde el cliente; cero invocaciones en Cloud Logging confirmó que el request nunca salía del WebView. F44.2 actualizó in-place el gotcha existente "CSP Firebase explícito" en [`gotchas/tauri-desktop.md`](gotchas/tauri-desktop.md) agregando `cloudfunctions.net` + nota distinguiendo callables (`httpsCallable`, requieren entry CSP) de triggers Firestore/scheduled (server-side, sin entry); D3 update vs gotcha nuevo → anchor preservado, índice de gotchas en este archivo sin cambios. F44.3 release coordinado v0.2.10 vía skill `release-ecosystem` (5 archivos bumpeados atómicamente: `package.json`, `package-lock.json`, `src-tauri/Cargo.toml`, `Cargo.lock`, `tauri.conf.json`; hosting deployado pre-tag a https://secondmind.web.app; tag `v0.2.10` disparó CI `release.yml` 9.08 min ambos jobs success: GitHub Release con MSI + NSIS + `latest.json` firmados, APK a Firebase App Distribution grupo `owner`). D1 subdominio específico vs wildcard `*.cloudfunctions.net` aceptado por consistencia con resto del CSP. Validación E2E en `tauri:dev` con query "inteligencia artificial" → 3 resultados temáticos con cosine similarity 37%/32%/31% y header "✨ Notas temáticamente similares". Hermana F39 (CSP `worker-src` para Reagraph): mismo perfil Tauri-only. Hallazgo paralelo NO escalado: warnings de Vite indican que fonts Geist (`geist-latin-wght-normal.woff2` + variants cyrillic/ext) "didn't resolve at build time" → woff2 no se copia al output del bundle, falla decode en runtime (`OTS parsing error: invalid sfntVersion: 1008813135` = `0x3C21444F` = `<!DO`, HTML del SPA fallback), cae a `font-sans` sin impacto visual perceptible — candidato a feature de polish independiente. Lección operativa escalable: `tauri-plugin-single-instance` puede sabotear smoke tests desktop si la app instalada (MSI/NSIS) está corriendo en el tray cuando se lanza `tauri:dev` — el plugin detecta segunda instancia y abre la ventana ya instalada en vez de la dev, DevTools muestra entonces bundle viejo + CSP viejo (síntoma "el fix no funciona" pero el error muestra el CSP de la versión instalada). Cierre completo desde tray (no solo cerrar ventana — close-to-tray mantiene proceso vivo) obligatorio pre-smoke-test desktop.
...
- **Feature 41 — Splash Tauri Desktop (hide-until-ready):** elimina el flash blanco entre que la window Tauri (Windows MSI/NSIS prod o `npm run tauri:dev`) aparece y que React + `<AppBootSplash>` termina el primer paint (~50–300ms cold start prod, hasta ~1–3s con bootstrap auth + hidratación TinyBase). Equivalente desktop del problema que F40 atacó en Android. Estrategia: main window arranca con `"visible": false` en `tauri.conf.json` y se revela post-paint via `showMainWindow()` desde `useHideSplashWhenReady` — el mismo hook F40 que dispara `hideSplash()` para Capacitor pasa a ser handoff cross-platform unificado (ambas calls no-op en web). Idempotency module-scope con flag `mainWindowShown` en `src/lib/tauri.ts` cubre StrictMode dev double-mount, `tauri-plugin-single-instance` `show()` desde Rust en 2da instancia mid-cold-start, y HMR full-reload (plugin Tauri es idempotente Rust-side). Safety timeout 5s en `main.tsx` paralelo al de Capacitor F40 cubre crash pre-mount Layout. 4 archivos editados (~22 líneas netas): `tauri.conf.json` agrega `visible:false`, `tauri.ts` agrega flag idempotency + try/catch defensivo, `useHideSplashWhenReady.ts` extiende a dual-platform, `main.tsx` agrega bloque `if(isTauri())`. Sin cambios Rust, sin `<AppBootSplash>` modifications, sin HTML inline en `index.html`. Estrategias B (show post-bootstrap) y C (HTML inline pre-React) descartadas en audit. Edge case aceptado: 2da instancia durante ~200ms de cold start, plugin `single_instance` llama `show()` ANTES que React monte → flash blanco breve (R3, mitigación a futuro). Gotcha escalado a `gotchas/tauri-desktop.md` (anchor `hide-until-ready-de-main-window-via-visiblefalse-showmainwindow-idempotente-post-f41`). → [SPEC](features/SPEC-feature-41-splash-tauri.md).

- **Feature 42 — Polish offline UX y visibilidad per-item:** cierra 5 deudas residuales tras F30 cuando el usuario reporta sensación de app rota offline. F42.1 hook `usePendingSyncForEntity(type, id): { isPending, hasError }` con `useSyncExternalStore` (subscribe app-lifetime + `getSnapshot` cached-by-`(entityType, id)` preservando ref si valores no cambiaron, `matchesId` para composite keys F30 D-bypass `${noteId}:accept-${suggestionId}`); helper `<PendingSyncDot>` reusable (`bg-amber-500` pending / `bg-destructive` error, posición `absolute right-1 top-1` default) montado en 5 cards (NoteCard, InboxItem, ProjectCard, ObjectiveCard, TaskCard con `right-12 top-1` para esquivar el margin negativo `-mr-2` del check button). F42.2 `<DiscardPendingDialog>` confirma antes de `clear()` queues con lista grouped por entidad (orden fijo note/inbox/task/project/objective/habit) + label resolvers per-entity (note→title, task/project/objective→name, habit→`Hábitos del DD/MM/YYYY`, inbox→rawContent truncado 80 chars); helper `getDiscardableEntries()` snapshot one-shot con dedupe `(entityType, id)` filtrando `status === 'synced'`; uso de `createsQueueBindings` para `delRow` ANTES de `clear()` (G4 F30, evita rows huérfanas en stores). F42.3 `<PendingSyncIndicator>` unificado con discriminated union `Severity = 'error' | 'offline' | 'pending'` + switches chipColors/dotColor; offline persiste el chip (azul) incluso sin pendings; `retryDisabled = errorCount === 0 || !isOnline`; popover header dinámico ("Sin conexión" vs "Cambios pendientes de sincronizar") + legend "Los cambios se sincronizarán al reconectar"; `<OfflineBadge>` deprecada (deleted) + import removido del Layout. F42.4 InboxItem en `src/components/capture/InboxItem.tsx` muestra placeholder "En cola — se procesará al reconectar" con `<CloudOff/>` cuando `!aiProcessed && !isOnline`, fallback al spinner "Procesando con AI..." cuando online. F42.5 Avatar fallback en `SidebarContent` con `useState<boolean>(false)` + `<img onError={() => setImgError(true)}>` + `useEffect(() => { setImgError(false); }, [user.photoURL])` para reset en logout/login; render condicional `{src && !imgError ? <img/> : <div className="bg-sidebar-primary"/>}`. 171/171 tests pasando (+10 vs F38: `usePendingSyncForEntity.test.ts` con 10 tests + 2 tests nuevos del PendingSyncIndicator para offline + dual states). E2E Playwright validado en 4 viewports (Desktop 1280, Tablet 768, Mobile 375, Samsung S23 Ultra 412×915). 2 polishes UX post-QA: suavizado destructive del dialog botón (sólido `bg-destructive` → `bg-destructive/10 hover:bg-destructive/20` + dark override `dark:bg-destructive/20 dark:hover:bg-destructive/30` siguiendo patrón `button.tsx:18-19`) y eliminación del Dialog.Close redundante (Cancelar + Escape + click backdrop ya cubren la afordancia). 2 gotchas escalados (avatar onError → `gotchas/ui-componentes.md`, hook per-entity → `gotchas/tinybase-firestore.md`). Limitación documentada D2 F42.1: items capturados via QuickCaptureProvider con `inboxStore.setRow` directo (path canónico D4 F38.3) NO van a queue → no se marcan como pending; escalable a F43 si surge demanda. → [SPEC](features/SPEC-feature-42-offline-ux-polish.md).

- **Feature 43 — Limpieza de navegación mobile (BottomNav 4 + drawer único):** elimina dos antipatrones de navegación mobile validados con literatura UX (NN/g, Material 3, Mobbin, Smashing). F43.1 CTA "Capturar" eliminado de `<SidebarContent />` shared cross-platform (desktop sidebar + tablet collapsed + mobile/tablet drawer afectados por construcción) + reorganización de `navItems.ts`: Inbox movido a "Ejecución" posición 2, sección "Captura" eliminada (regla "ninguna sección de drawer con un único hijo"). F43.2 `<BottomNav />` 5→4 slots (Dashboard / Notas / Tareas / Inbox), slot "Más" + prop `onMoreClick` + `<MoreDrawer />` eliminados (componente borrado, sin tests, sin consumers fuera de `app/layout.tsx`). FAB persiste como única primary action mobile; position invariante al slot count via altura BottomNav fija. 171/171 tests, validado E2E en mobile 390×844 (4 slots × 98×63px WCAG-compliant) + tablet 768 + desktop 1280. D6 trade-off: tablet (768-1023px) y desktop con sidebar visible quedan con sólo shortcut Alt+N para QuickCapture (TopBar sólo con `sidebarHidden=true`, FAB sólo mobile); aceptado por uso minoritario, escalable a F44 si surge feedback de discoverability. Cleanup mecánico sin gotchas técnicos escalados; lecciones archivadas en el SPEC. → [SPEC](features/SPEC-feature-43-nav-mobile-cleanup.md).

- **Feature 44 — Fix CSP `cloudfunctions.net` Tauri (búsqueda híbrida desbloqueada):** la CSP de `tauri.conf.json` omitía `cloudfunctions.net` donde viven las CFs callables (`embedQuery` desde F3 búsqueda híbrida); el WebView2 bloqueaba el `httpsCallable` por violación de `connect-src` antes de salir, devolviendo "Sin resultados" silencioso en `/notes` de la app desktop. Bug Tauri-only (PWA web y Capacitor Android sin CSP custom), latente desde F3 porque `embedQuery` era la única callable consumida desde el cliente hasta esta feature; cero invocaciones en Cloud Logging confirmó que el request nunca salía del WebView (señal positiva para CSP-bloqueado: discriminar cliente vs servidor en bugs de CF). Fix mínimo en `src-tauri/tauri.conf.json:43` agregando `https://us-central1-secondmindv1.cloudfunctions.net` al `connect-src` en el bloque Firebase (post `securetoken`, pre `www.googleapis.com`) — D1 subdominio específico vs wildcard `*.cloudfunctions.net` aceptado por consistencia con resto del CSP. Gotcha existente "CSP Firebase explícito" en `gotchas/tauri-desktop.md` actualizado in-place agregando `cloudfunctions.net` + nota distinguiendo callables (httpsCallable, requieren entry CSP) de triggers Firestore/scheduled (server-side, sin entry) — D3 update in-place vs gotcha nuevo, anchor preservado. Release coordinado v0.2.10 (skill `release-ecosystem`): hosting pre-tag a https://secondmind.web.app, CI `release.yml` 9.08 min ambos jobs success (MSI + NSIS + `latest.json` firmados + APK a Firebase App Distribution grupo `owner`). Validación E2E en `tauri:dev` con query "inteligencia artificial" → 3 resultados temáticos similarity 37%/32%/31% y header "✨ Notas temáticamente similares". Hermana de F39 (CSP `worker-src` para Reagraph): mismo perfil Tauri-only — bugs CSP quedan latentes hasta que un flow los invoca (cualquier nueva integración externa con WebView2 — API third-party, CF callable, worker blob, CDN font, OAuth endpoint — debe testearse explícito en `tauri:dev` ANTES del próximo release coordinado). Lección operativa escalable: `tauri-plugin-single-instance` puede sabotear smoke tests desktop si la app instalada está corriendo en el tray cuando se lanza `tauri:dev` (plugin detecta 2da instancia y abre la ventana ya instalada, DevTools muestra bundle viejo + CSP viejo). Hallazgo paralelo NO escalado: fonts Geist (`geist-latin-wght-normal.woff2` + variants cyrillic/ext) no resuelven en build Vite (warning "didn't resolve at build time"), woff2 falla decode en runtime con HTML del SPA fallback (`OTS parsing error sfntVersion 0x3C21444F = <!DO`), cae a `font-sans` sin impacto visual perceptible — candidato a feature de polish independiente. Sin gotchas escalados al índice (D3 update in-place del existente). → [SPEC](features/SPEC-feature-44-csp-cloudfunctions-tauri.md).
...
- [Hide-until-ready de main window via `visible:false` + `showMainWindow()` idempotente (post-F41)](gotchas/tauri-desktop.md#hide-until-ready-de-main-window-via-visiblefalse-showmainwindow-idempotente-post-f41)
