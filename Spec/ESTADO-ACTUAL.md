...
> Última actualización: Feature 11 Store Isolation + Gating (Abril 2026)
...
- **Write amplification del persister (pre-existente, no resuelto por F10/F11).** El callback `setPersisted` en `src/lib/tinybase.ts:50-59` itera TODAS las rows de la tabla en cada save. Con 500 items y 1 update, son 500 writes a Firestore adicionales al write explícito del factory. Feature 12 candidata resolverá esto con diff-based persister.
- **Cross-user data leak resuelto por F11 (`store.delTable` pre-init + post-destroy en useStoreInit).** Cualquier nueva tabla TinyBase agregada al `configs` array de [src/hooks/useStoreInit.ts](../src/hooks/useStoreInit.ts) hereda automáticamente el cleanup. Orden crítico en cleanup: `destroy()` (apaga onSnapshot + autoSave) antes de `delTable()`; invertido = race con snapshot in-flight repoblando la tabla vacía. delTable con persister snapshot-based NO borra Firestore (Object.entries({}).map() → Promise.all([]) → cero setDoc).
...
- **Capacitor 8 exige Node >=22 y JDK 21 en el runner Android.** `setup-node@v4` con `node-version: 22` (Capacitor CLI requirement) y `setup-java@v4` con `java-version: 21` (Capacitor genera `capacitor.build.gradle` con `sourceCompatibility=21`). Local pasa con 22 + JBR de Android Studio 2024+ pero los defaults del SPEC (20 + 17) fallan. `release-capacitor` queda asymétrico con `release-tauri` en Node 20 — aceptable, tauri-action no invoca Capacitor CLI.
...
- **Feature 12 — Persister diff-based** (única candidata pendiente del backlog de auditoría F10). Resuelve el write amplification del persister: `setPersisted` itera toda la tabla en cada save (500 items + 1 update = 500 setDocs). Baja prioridad hasta dogfooding con >200 items/user. Sin SPEC todavía.
