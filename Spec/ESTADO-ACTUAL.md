...
- **`data-tauri-drag-region`** en el header de `/capture` (32px, `h-8`) permite arrastrar ventana frameless sin JS. Habilitado por la capability `core:window:allow-start-dragging` (Feature 7). Drag limitado al header a propósito — full window aumentaría riesgo de drag accidental en textarea/footer.
...
- **`onScaleChanged` listener en `CapturePage` para revertir DPI drift durante drag (fix post-F7 round 2).** El fix previo (setSize post-show) solo corregía en la próxima invocación, no durante el drag activo que es cuando aparece el síntoma. Pattern correcto: `window.onScaleChanged(async () => await window.setSize(new LogicalSize(W, H)))` responde al mismo evento (`WM_DPICHANGED`) que dispara la corrupción. Requiere capability `core:window:allow-set-size` en `capture.json`. Aplica a cualquier ventana frameless que sea draggeable.
...
32. **WebView2 no rescala contenido tras cross-DPI drag (fix post-F7).** Tauri/tao handlea WM_DPICHANGED para el chrome de la ventana (mantiene LogicalSize entre monitores), pero WebView2 no recibe el hint para rescalar su contenido interno. Resultado visible: scrollbars horizontales + verticales y clipping del UI tras arrastrar entre monitores con DPI distinto. Issue upstream: tauri-apps/tauri#3610 (abierto desde 2022, sin fix). Workaround: **`onScaleChanged` listener en la ventana frameless** que llama `setSize(LogicalSize(W, H))` al instante — responde al mismo evento que causa la corrupción, en vez de "próxima vez que se invoque". Aplica a cualquier ventana frameless (`decorations: false`) que pueda cruzar monitores heterogéneos.
