...
> Última actualización: Feature 9 Capacitor Auto-Update (Abril 2026)
...
- **Write amplification del persister (pre-existente, no resuelto por F10).** El callback `setPersisted` en `src/lib/tinybase.ts:50-59` itera TODAS las rows de la tabla en cada save. Con 500 items y 1 update, son 500 writes a Firestore adicionales al write explícito del factory. Feature 12 candidata resolverá esto con diff-based persister.
- **Cross-user data leak potencial (pre-existente).** Los stores TinyBase son singleton; `useStoreInit` no llama `store.delTable` antes de `startAutoLoad` en user switch. Ventana <100ms entre logout/login donde un click del user podría escribir rows del user viejo al path del user nuevo. Feature 11 candidata (prioridad pronta) resolverá esto.
