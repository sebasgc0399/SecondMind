...
## DMARC publicado — `p=none` (observación); `rua` a destino externo (gmail) no autorizado

`getsecondmind.co` **tiene DMARC publicado** (Cloudflare, **DNS-only**), verificado por DNS el **2026-06-22** (`Resolve-DnsName` contra **1.1.1.1 y 8.8.8.8** — `dig` no está en este Windows): **un solo** `_dmarc`, **sin duplicado**, valor `v=DMARC1; p=none; rua=mailto:sebasgc0399@gmail.com; fo=1`. Root **intacto** (SPF `v=spf1 include:_spf.mx.cloudflare.net ~all`, MX `route1/2/3.mx.cloudflare.net` de Email Routing — **sin DMARC mezclado en el apex**). `p=none` = **modo observación**: no cuarentena/rechaza nada, solo solicita reportes agregados. SPF+DKIM ya autentican el From, así que DMARC acá es reputación + telemetría, no enforcement. **Endurecer a `quarantine`/`reject` post-beta**, una vez que los reportes `rua` confirmen que no hay fuentes legítimas sin alinear.

**Caveat (follow-up abierto, no bloqueante):** el `rua` apunta a `gmail.com`, **dominio externo** a `getsecondmind.co`. La **RFC 7489 §7.1** ("Verifying External Destinations") exige que el destino externo publique un record de autorización en `getsecondmind.co._report._dmarc.gmail.com` (`TXT "v=DMARC1"`); ese record **NO existe** (verificado NXDOMAIN 2026-06-22) y Gmail **no lo publica** para mailboxes de usuario arbitrarios. Consecuencia: los reporters que cumplen la RFC **pueden ignorar el destino** y no enviar los reportes agregados → el plan "endurecer con datos de los reportes `rua`" queda **sin datos garantizados**. **Fix limpio y ya viable** (Email Routing inbound activo): mover a `rua=mailto:dmarc@getsecondmind.co` y rutear `dmarc@` → gmail por Cloudflare Email Routing → `rua` **in-domain** (mismo org-domain que el `_dmarc`), sin autorización externa, y los reportes igual caen en el inbox.
