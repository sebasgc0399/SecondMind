...
> **Reincidencia (consent server-authoritative, 2026-06).** Volvió a morder en el **runtime del trigger desplegado** `onSemanticConsentChanged` (no en un E2E directo): el `serverTimestamp()` del log de evidencia con `admin.firestore.FieldValue` tiraba `Cannot read properties of undefined (reading 'serverTimestamp')`, mientras que la purga del MISMO trigger escribía bien (no usa estáticos). **Modo de falla NUEVO — falso verde por doble ocultamiento:** (1) `sanitizeError` logueaba el mensaje sanitizado, no el `TypeError`; (2) el test del trigger inyectaba la escritura del log como **spy** (DI), así que la función REAL `appendConsentStateEvent` nunca corría contra el emulador → 62/62 verde con el bug vivo. **Lección de QA:** un gate/efecto crítico hay que ejercerlo con la función REAL contra el emulador, no solo via spy/DI (el spy da confianza falsa). El **mismo bug latente** vivía en el callable `markSemanticConsent` (no testeado en runtime hasta agregar su handler e2e) y ahí habría roto **todo el `batch.commit` del ack en prod**, no solo el log best-effort. Fix: `import { FieldValue } from 'firebase-admin/firestore'` en `consentLog.ts` + `markSemanticConsent.ts`. **Regla reforzada: para CUALQUIER `serverTimestamp`/`Timestamp`/`increment` nuevo en functions, usar SIEMPRE el import modular — sin excepción.** Vivo en [consentLog.ts](../../src/functions/src/lib/consentLog.ts) + [markSemanticConsent.ts](../../src/functions/src/settings/markSemanticConsent.ts).
