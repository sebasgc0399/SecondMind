...
## Avatar `<img>` con URL remota: onError + state fallback obligatorio (post-F42.5)

`<img src={remoteURL}>` para photoURL de auth providers (Google/Microsoft) o signed URLs de Firebase Storage puede fallar silenciosamente: CORS bloqueado en Capacitor WebView con `referrerPolicy="no-referrer"`, redirect chain de Google que cambia el host fuera del CSP, network mid-load, signed URL expirada. Sin handler, el browser pinta el broken-image placeholder con el alt text dentro del contenedor — layout shift visible (en `h-8 w-8` con texto "Sebastian" superpuesto al ícono X) que dice "app rota" al usuario. Patrón canónico: `const [imgError, setImgError] = useState(false)` + `<img onError={() => setImgError(true)} ...>` con render condicional `{src && !imgError ? <img/> : <Fallback/>}`. **`useEffect(() => { setImgError(false); }, [src])` obligatorio** para resetear el flag cuando la URL cambia (logout → login con otra cuenta, sino el avatar de la cuenta nueva nunca aparece). Aplicable a cualquier `<img>` con URL remota: avatares de auth, attachments de Firebase Storage, thumbnails generados, OG images, etc. Patrón vivo en [src/components/layout/Sidebar.tsx](../../src/components/layout/Sidebar.tsx) (`SidebarContent`).

## Tokens `design-system/secondmind/MASTER.md` desincronizados del runtime `src/index.css` (post-F47)

MASTER.md documenta tokens como `--warning` (`oklch(0.78 0.18 75)`), `--foreground-muted`, `--background-deep` que el runtime real `src/index.css` no expone — o los expone con nombres distintos (`--muted-foreground` es el nombre real, no `--foreground-muted`). Razón histórica: MASTER.md fue escrito en Fase 0 como aspiracional, el setup de Tailwind v4 `@theme inline` se construyó incrementalmente pegando lo necesario y nunca se backportearon todos los tokens del Master. Resultado: usar `bg-warning/10` en JSX falla silenciosamente (Tailwind v4 trata clases desconocidas como no-op) y el visual aparece como plano transparent. **Antes de usar cualquier clase de color que no sea `primary/destructive/foreground/background/muted/border/popover/card`**, verificar con `grep "<token-name>" src/index.css` que el token existe en el bloque `@theme inline` o en `:root`/.dark. Si falta: (a) agregarlo al `@theme inline` siguiendo el patrón existente (CSS variable + entry mapping), O (b) usar el color de Tailwind directo (`bg-amber-500`, `bg-green-600`, precedente vivo en [src/components/tasks/TaskCard.tsx](../../src/components/tasks/TaskCard.tsx) y [src/components/projects/ProjectCard.tsx](../../src/components/projects/ProjectCard.tsx) que usan `amber-500` para warnings UI sin pasar por token semántico). Anti-patrón: confiar ciegamente en MASTER.md como referencia de tokens disponibles. Patrón vivo en [src/components/auth/EmailVerificationBanner.tsx](../../src/components/auth/EmailVerificationBanner.tsx) (`bg-amber-500/10 border-amber-500/30 text-amber-700 dark:text-amber-300`).

**Actualización (chore/design-principles):** `MASTER.md` fue **eliminado** del repo — `src/index.css` es ahora la única fuente de tokens (sin doc paralelo que pueda divergir). La lección operativa queda intacta: verificar el token con `grep "<token-name>" src/index.css` antes de usar una clase de color, y usar Tailwind directo (`amber-500`) para warnings sin token semántico. El criterio de diseño que consume el agente `design-review` vive en [`.claude/design-principles.md`](../../.claude/design-principles.md).

## Descarga client-side con Blob y anchor download: cleanup diferido, no sincrónico

Para descargar un archivo generado en el cliente (export a zip, CSV, etc.) el patrón es `new Blob(...)` → `URL.createObjectURL` → `<a download={filename}>` → `anchor.click()`. **Trampa:** remover el anchor (`anchor.remove()`) o revocar el object URL **sincrónicamente** justo después del `.click()` rompe el nombre en **Chrome real** — el browser lee el atributo `download` un tick DESPUÉS del click, y si para entonces el anchor ya no está en el DOM, cae al nombre del blob URL (un **UUID, sin extensión**) → el usuario baja un archivo que no puede abrir.

**Fix (patrón FileSaver):** cleanup **DIFERIDO** — `setTimeout(() => { anchor.remove(); URL.revokeObjectURL(url); }, 1000)` para que el anchor sobreviva lo que el browser tarda en procesar la descarga. Sumar `anchor.style.display = 'none'` + `anchor.rel = 'noopener'`. Detectado y fixeado en SPEC-67 (export); patrón vivo en [`src/lib/export/deliverExport.ts`](../../src/lib/export/deliverExport.ts) (`triggerZipDownload`).

**Por qué el QA inicial NO lo cazó:** Playwright captura el `suggestedFilename` vía CDP en el instante del click → el QA automatizado mostraba el nombre correcto y enmascaraba el bug; solo apareció al mirar el archivo bajado en un browser real. Lección de método canonizada en [`gotchas/tooling-local.md`](tooling-local.md#qa-de-descargas-client-side-playwright-via-cdp-enmascara-bugs-de-naming). Aplica a cualquier feature que genere y descargue un archivo client-side.
