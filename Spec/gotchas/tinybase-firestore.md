...
## Rutas pre-auth: TinyBase no inicializado → `getDoc` directo del SDK web (post-F47)

Cualquier hook que corra en rutas pre-auth (LoginPage, reset password, sign-up, account recovery, error pages mostradas antes de auth gate) NO puede usar TinyBase: sus persisters dependen del UID (`useStoreInit(user?.uid ?? null)`) y permanecen sin inicializar hasta que `onAuthStateChanged` emite un user. Cualquier feature que requiera leer Firestore en estas rutas debe hacer `getDoc(doc(db, '<path>'))` directo del SDK web (`firebase/firestore`), con cache propio (`sessionStorage` con TTL) si la lectura es repetitiva. Esto incluye lectura de `config/app` o `config/*` para feature flags / capacity gates / waitlist configs. **Documentar el bypass al tope del hook** con comentario explícito justificando la decisión, para evitar que un "fix bien intencionado" futuro migre a TinyBase y rompa la pantalla sin que sea obvio en code review (la rotura es invisible hasta probar el flow signed-out). Las rules deben permitir `allow read: if true` (no `request.auth != null`) sobre el path porque no hay user al hacer el query — verificar que el contenido del doc no exponga PII. Patrón vivo en [src/hooks/useSignupCapacity.ts](../../src/hooks/useSignupCapacity.ts) (lee `config/app` con `getDoc` + cache 60s sessionStorage).
