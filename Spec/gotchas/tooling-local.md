...
## Hooks de Windows que tocan `file_path`: el separador es BACKSLASH (normalizar con `tr`, no parameter expansion)

Un hook de Claude Code recibe el path por stdin JSON (`.tool_input.file_path`); en Windows, `jq -r '.tool_input.file_path'` lo devuelve con **`\` (backslash)** — confirmado empíricamente con un probe temporal (`echo "[$(jq -r '.tool_input.file_path')]" >> log` + un edit de prueba → `[D:\Proyectos VS CODE\SecondMind\...]`, sin `^M`). Implicaciones para CUALQUIER hook que derive el **directorio** del archivo a partir de `file_path`:

- **Parameter expansion NO recorta el backslash.** `${fp%[/\\]*}` falla (el bracket-class `[/\\]` no matchea ningún separador en bash 5.2.x) y el backslash es intratable por `${p%\\*}` / `${p//\\//}` (se consume como escape antes del match). Cualquier fix que dependa de strip de backslash por parameter expansion es frágil por diseño.
- **Normalizar el separador con `tr`** (sí maneja backslash) ANTES de recortar el dir: `d=$(printf '%s' "$fp" | tr '\\' '/'); d="${d%/*}"`. En el JSON de `settings.json`, `tr` se escribe con **CUATRO** backslashes (`tr '\\\\' '/'`) → jq entrega DOS al shell-string → `tr` interpreta UN backslash, **sin** el warning "unescaped backslash ... is not portable". Convención de escaping del propio archivo (mismo patrón que el hook de gotchas-reindex). Verificar por **byte-count**, no por render del terminal: archivo = 4, shell-string (`jq -r`) = 2.
- **Rama del worktree real:** `git -C "$d" rev-parse --abbrev-ref HEAD 2>/dev/null` resuelve la rama del **dir del archivo**, NO la del cwd del proceso del hook (que es el repo principal). Sin `-C`, `git branch --show-current` lee la rama del principal → en un worktree con rama feature el guard de protección de `main` bloqueaba edits legítimos (bug F3.2: principal en main + worktree en feature → el hook leía `main` y bloqueaba el edit del worktree).
- **Fail-closed ante ambigüedad:** `[ -n "$br" ] && [ "$br" != "main" ] || exit 2`. Un dir no-repo hace `git -C` exit 128 → `$br` vacío; un hook de PROTECCIÓN bloquea ante la duda, no permite.
- **Corolario operativo (F59):** crear el **primer archivo en un subdirectorio NUEVO** (aún inexistente en disco) dispara el mismo fail-closed — `git -C <dir-nuevo>` exit 128 → `$br` vacío → BLOCK, aunque la rama sea feature. Workaround: `mkdir -p <dir>` ANTES del primer `Write` (el dir queda dentro del repo → `git -C` resuelve la rama feature → ALLOW). Pasó al crear `src/components/changelog/WhatsNewModal.tsx`.

⚠️ **Un hook anotado mal degrada la protección a NO-OP — peor que el bug.** El fix originalmente propuesto (`git -C "${fp%[/\\]*}" rev-parse ...`) pasaba la ruta de un ARCHIVO a `git -C` (la expansión rota no recortaba el dir) → exit 128 → `$br` vacío → `[ "" != "main" ]` TRUE → ALLOW siempre. La protección de `main` quedaba **desactivada** sin que nada lo delatara.

**Verificación obligatoria por exit code, no por afirmación:** extraer el comando REAL del archivo (`jq -r '.hooks.PreToolUse[0].hooks[0].command' settings.json`), alimentarlo con stdin simulado (`jq -n --arg fp "$path" '{tool_input:{file_path:$fp}}'`, que replica el escaping del harness) y correr cada caso × separador. Casos mínimos para un guard de rama: dir-en-main → exit 2 (BLOCK), worktree-en-feature → exit 0 (ALLOW), dir-no-repo → exit 2 (fail-closed); en backslash (el real) **y** forward. Worktree de test real: `git worktree add -b <rama> "<path-con-SecondMind-en-el-nombre>"` (el nombre del dir debe matchear el substring-guard del `case`); limpieza `git worktree remove --force` + `git worktree prune` + `git branch -D`.

**Reload:** los hooks de `settings.json` se recargan en vivo (file-watcher; un probe se activó al agregarlo y se desactivó al revertirlo, sin reinicio). `/hooks` da la confirmación visual del comando cargado. Aplicar el fix del propio `settings.json` requiere estar en rama feature: el guard se autoaplica (el archivo está bajo el path protegido), pero el self-block se escapa con `git checkout -b` ANTES de editar — `checkout` no está hookeado. Vivo en [`.claude/settings.json`](../../.claude/settings.json) (PreToolUse). Detectado en ITEM 2 del backlog post-i18n.

## QA de descargas client-side: Playwright via CDP enmascara bugs de naming

Playwright intercepta las descargas vía CDP y expone el `suggestedFilename` que lee del atributo `download` en el **instante del click**. Por eso un QA de descargas SOLO con Playwright reporta el nombre correcto **aunque el browser real lo pierda** (ej. el bug de cleanup sincrónico del anchor → archivo con UUID sin extensión; ver [`gotchas/ui-componentes.md`](ui-componentes.md#descarga-client-side-con-blob-y-anchor-download-cleanup-diferido-no-sincrónico)). El download event de Playwright (`page.on('download')` / el "Downloaded file X to ..." del MCP) usa ese `suggestedFilename`, no el nombre con que el browser realmente guardaría **sin** interceptación.

**Regla:** para cualquier feature que GENERE Y DESCARGUE un archivo, el QA automatizado con Playwright valida el **contenido y el flujo**, pero NO valida el **naming real del browser** — hay que **mirar el archivo bajado en un browser real** (el de uno, o pedirle al usuario el screenshot de sus Descargas) o aceptar que ese aspecto queda sin cubrir. Detectado en SPEC-67: el smoke Playwright del export mostró `secondmind-export-2026-06-24.zip` correcto, pero en el Chrome real del usuario bajaba con el UUID del blob — solo se cazó cuando el usuario abrió sus Descargas. Generalizable: **Playwright es fuerte para contenido/flujo, ciego para el naming de descarga del browser sin interceptar.**
