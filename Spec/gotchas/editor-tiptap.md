...
## Close-on-scroll de `useEditorPopup` ignora el scroll interno del menú; keyboard nav scrollea el item seleccionado

Dos comportamientos **acoplados** de [`useEditorPopup`](../../src/components/editor/hooks/useEditorPopup.ts) (los consumen SlashMenu, WikilinkMenu y cualquier popup futuro). El menú tiene su propio contenedor scrollable interno (`max-h-* overflow-y-auto`). **(A)** El handler de close-on-scroll (capture phase, que cierra al scrollear el documento) debe IGNORAR los scrolls cuyo `event.target` esté dentro de `menuRef.current` — si no, scrollear con rueda/touch dentro del menú lo cierra. Solo el scroll del documento/editor (que mueve el anchor fuera de vista) debe cerrar. **(B)** La navegación con flechas necesita un `useLayoutEffect` sobre `[selectedIndex, isOpen, items]` que haga `querySelector('[aria-selected="true"]').scrollIntoView({ block: 'nearest' })`, o el item seleccionado queda fuera de la ventana visible al bajar. El acople: el `scrollIntoView` de (B) dispara el handler de (A), así que ignorar el scroll interno (A) es **precondición** de (B). `block: 'nearest'` scrollea solo el contenedor interno (el popup es `position: fixed`, ya dentro del viewport); `selectedIndex` es índice GLOBAL aun en el slash menu agrupado por categoría, así que el `querySelector` de `aria-selected` encuentra el botón correcto. **Distinto** del scroll PROGRAMÁTICO que ProseMirror dispara al abrir (ver "Scroll listener tras char trigger requiere defer 1 task tick"): aquel es el scroll de apertura; este es el scroll del usuario dentro del menú ya abierto. Fix en [`useEditorPopup.ts`](../../src/components/editor/hooks/useEditorPopup.ts) (`handleScroll` + el `useLayoutEffect` de `scrollIntoView`), commit `5e11ee4`.
...
## Tablas via `TableKit` de `@tiptap/extension-table` (v3 — un solo paquete)

TipTap v3 expone tablas en **un único paquete** `@tiptap/extension-table` que exporta `TableKit` (agrupa `Table` + `TableRow` + `TableHeader` + `TableCell`, igual que `StarterKit`). NO son paquetes separados como en v2. Setup: `TableKit.configure({ table: { resizable: true } })`. Comandos: `insertTable({ rows, cols, withHeaderRow })`, `addColumnBefore/After`, `deleteColumn`, `addRowBefore/After`, `deleteRow`, `deleteTable`, `toggleHeaderRow/Column`, `mergeCells`/`splitCell`/`mergeOrSplit`. **El CSS de ProseMirror es OBLIGATORIO** (`.note-editor .ProseMirror table/td/th/.selectedCell/.column-resize-handle/.tableWrapper`) — sin él la tabla no muestra bordes ni el handle de resize. Themed con variables semánticas (`--border`/`--muted`/`--primary`) que ya cambian en `.dark` → no hace falta bloque `.dark` dedicado. Persistencia/search/embeddings **sin cambios**: el doc JSON va entero a Firestore y `contentPlain` (`getText()`) ya incluye el texto de celdas. Patrón vivo en [NoteEditor.tsx](../../src/components/editor/NoteEditor.tsx) + [index.css](../../src/index.css). F62.

## El wrapper flotante de `BubbleMenu` es `z-index: auto` → el sidebar (z-30) lo tapa

El elemento posicionado (`position: absolute`) que `@tiptap/react/menus` `BubbleMenu` crea para el floating content tiene `z-index: auto`, y ningún ancestro del editor crea stacking context. Cuando el menú se solapa con el sidebar (`z-30`) — pasa con tablas/celdas cerca del borde izquierdo del contenido — el sidebar lo **tapa visualmente E intercepta los clics** (los botones quedan inalcanzables, "el menú no funciona"). Fix: `relative z-50` en el `<div>` raíz del contenido del menú — un hijo con `z-index` sube al stacking context raíz porque el wrapper absolute z-auto no crea contexto, y gana sobre el sidebar. Aplica a CUALQUIER BubbleMenu sobre contenido que pueda solaparse con chrome de mayor z. **Ambos toolbars del editor ya llevan `relative z-50`**: el `TableToolbar` (F62) y el `BubbleToolbar` de formato (cierre de deuda post-F62 — su wrapper `z-50` es común a los 3 modos: link-edit / link-hover / formato). El z-50 **no rompe stacking con otros overlays**: al abrir un modal (CommandPalette/WelcomeModal/WhatsNewModal) el BubbleMenu se oculta porque el editor pierde el foco → no asoma sobre el backdrop (z-40), y de todas formas el popup del modal (z-50 en portal, al final del DOM) gana por orden de pintado; los popups del propio editor (SlashMenu/WikilinkMenu, z-60) son mutuamente exclusivos con el BubbleToolbar. Smoke E2E verificado en el cierre de la deuda.

## Toolbar flotante de muchos botones: `flex-wrap` + `max-w-[calc(100vw-1rem)]` para mobile

Un BubbleMenu con muchos botones (el `TableToolbar` tiene 13: filas/columnas/header/alineación/merge/borrar) mide ~640px y **se sale del viewport en mobile** (<400px), dejando botones fuera de pantalla inaccesibles — el menú "no aparece" desde la perspectiva del usuario. Fix: `flex-wrap` + `max-w-[calc(100vw-1rem)]` en el contenedor + `shrink-0` en cada botón/divider → envuelve a 2-3 filas dentro del viewport (en 375px: 359px / 2 filas). En desktop no cambia (hay espacio, no envuelve). F62.

## El walker recursivo de `extractLinks` recorre celdas de tabla (wikilinks dentro de celdas NO se dropean)

Complemento del walker genérico (ver "Marks viven en text nodes"): como `extractLinks` recurre sobre `node.content` sin código específico por tipo, un `wikilink` dentro de `table → tableRow → tableCell → paragraph` se extrae igual que en cualquier bloque. El `context` del link es el texto del párrafo de la celda (bloque no-inline más cercano). **Verificado E2E en F62** (no solo unit test): un wikilink en una celda crea el doc en `links/` y registra el backlink en `incomingLinkIds` de la nota target. Cualquier futuro nodo-contenedor nuevo hereda esto sin tocar `extractLinks`. Guardrail en [extractLinks.test.ts](../../src/lib/editor/extractLinks.test.ts).

## TextAlign (F62) es global: aplica a todo `paragraph`/`heading` + registra atajos de teclado

`@tiptap/extension-text-align` con `types: ['heading', 'paragraph']` permite `textAlign` en CUALQUIER párrafo (no solo en celdas) y registra atajos `Mod-Shift-l/e/r/j` por default. Los botones de alineación viven solo en el `TableToolbar`, pero los atajos funcionan en todo el editor — la alineación global es intencional (decisión D3 de F62), no un efecto colado. Restringirla solo a celdas no es trivial: el párrafo de una celda es el mismo nodo-tipo que uno normal, así que habría que abandonar TextAlign por un mark/attr custom de celda.
